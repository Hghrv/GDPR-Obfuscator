...
def mock_s3_bucket():
    ...
    with mock_aws():
        ...
        s3.put_object(Bucket=bucket_name,
                      Key="new_data/test_file.csv", Body=csv_content)
...
class TestGetFileFromInputS3:
    ...
    def test_get_file_from_input_s3_logs_error(self):
        ...
        with LogCapture() as log:
            error_event = get_file_from_input_s3(
                "not-a-bucket", "src/test_file.csv")
...
class TestLoadIntoDataframe:
    ...
    def test_load_csv_into_dataframe(self, mock_s3_bucket):
        ...
        load = load_csv_into_dataframe(
            bucket_name=test_bucket_name, file_key=test_file_key)
    ...
    def test_load_csv_into_dataframe_logs_error(self):
        with LogCapture() as log:
            error_event = load_csv_into_dataframe(
                "not-a-bucket", "src/test_file.csv")
        assert error_event["result"] == "Failure"
        assert "An error has occured with the client:" in str(log)
