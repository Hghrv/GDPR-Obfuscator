...
class TestUploadToOuputS3:
    def test_upload_to_ouput_s3_returs_dictionary(self, mock_s3_bucket):
        ...
        upload_test = upload_to_ouput_s3(
            bucket_name_output, output_key, file_to_upload)
        # Asserting that the function returns a dictionary
        assert isinstance(
            upload_test, dict), "response is not a dictionary instance"
    ...
    def test_upload_to_ouput_s3(self, mock_s3_bucket):
        ...
        response = upload_to_ouput_s3(
            mock_s3_bucket, output_key, file_to_upload)
    ...
    def test_handles_no_such_bucket_error(self):
        ...
        with LogCapture() as log:
            output = upload_to_ouput_s3(
                "non-existant-bucket", "obfuscated_data/obfuscated-file.csv", "src/test_file.csv")
    ...
    def test_handles_filename_error(self, mock_s3_bucket):
        ...
        with LogCapture() as log:
            output = upload_to_ouput_s3(
                "{output_s3}", "test-key", "non-existant-file.csv")
            assert output["result"] == "Failure"
