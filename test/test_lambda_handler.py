...
class TestObfuscationOfCsvFiles:
    ...
    def test_lambda_handler_downloads_csv_files_from_input_s3_bucket(self):
        ...
        s3_client.put_object(Bucket=bucket_name,
                             Key=input_test_key, Body=test_file)
    ...
    def test_lambda_handler_uploads_csv_files_to_output_s3_bucket(self):
        ...
        response = s3_client.list_objects(
            Bucket=bucket_name_output, Prefix="obfuscated_data")
    ...
    def test_lambda_handler_writes_output_csv_file_into_bytestream(self):
        ...
        response = s3_client.get_object(
            Bucket=bucket_name_output, Key=output_key)

        # Assert that reponse is bytes-like
        assert isinstance(response['Body'].read(
        ), (bytes, bytearray)), "Content of stream_data is not bytes-like"
...
class TestObfuscatorForJsonAndParquetFileTypes:
    ...
    def test_lambda_handler_handles_json_files_and_output_has_expected_obfuscated_json_content(self):
        ...
        s3_client = boto3.client('s3')
        s3_client.put_object(Bucket=bucket_name,
                             Key=input_test_key, Body=json_data)
    ...
    def test_lambda_handler_handles_parquet_files_and_output_has_expected_obfuscated_parquet_content(self):
        ...
        print(df)
        # Converting DataFrame to Parquet format in memory
        buffer = BytesIO()
        table = pa.Table.from_pandas(df)
        pap.write_table(table, buffer)

        parquet_bytes = buffer.getvalue()
        # Uploading to input S3
        s3 = boto3.client('s3')
        ...
        s3.put_object(Bucket=bucket_name,
                      Key=input_test_key, Body=parquet_bytes)

        # s3_client = boto3.client('s3')
        # s3_client.put_object(Bucket=bucket_name, Key=input_test_key, Body=test_file)

        lambda_handler(json_event, aws_context)   # Calling the lambda function
        response = s3.get_object(Bucket=bucket_name_output, Key=output_key)
        # response_buffer = response["Body"].read().decode("utf-8")

        parquet_output = BytesIO(response["Body"].read())

        # Reading the Parquet data in a Pandas DataFrame
        extracted = pap.read_table(parquet_output).to_pandas()
        # df_1 = pd.read_parquet(parquet_output, dtype_backend="pyarrow")
        df_1 = extracted.iloc[0, 0]
        print(df_1)
        print(type(df_1))
        assert len(df_1) == 6
        assert df_1 == {'cohort': "'December'", 'course': "'Software'", 'email_address': "'***'",
                        'graduation_date': "'2024-03-31'", 'name': "'***'", 'student_id': '1234'}
        assert df_1["student_id"] == '1234'
        assert df_1["name"] == "'***'"
        assert df_1["course"] == "'Software'"
        assert df_1["cohort"] == "'December'"
        assert df_1["graduation_date"] == "'2024-03-31'"
        assert df_1["email_address"] == "'***'"
